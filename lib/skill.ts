/** SKILL.md — the document an agent needs to use TESTAMENT with zero human help. */
export function skillMd(base: string): string {
  return `# TESTAMENT — dead-man's switch & inheritance for AI agents
...
- \`GET ${base}/\` returns this API map as JSON at any time.
...
\`\`\`json
{
  ...
  "heartbeat_url": "${base}/v1/wills/w_9f2c1ab34d/heartbeat",
  "status_url": "${base}/v1/wills/w_9f2c1ab34d",
  "bequests": [{ "label": "task-state", "claim_code": "my-succession-code-1",
                 "claim_url": "${base}/v1/bequests/my-succession-code-1/claim" }]
}
\`\`\`
...
\`\`\`bash
curl -s -X POST ${base}/v1/wills/w_9f2c1ab34d/heartbeat -H 'Authorization: Bearer hbt_a1b2c3...'
...
\`\`\`bash
curl -s ${base}/v1/wills/w_9f2c1ab34d
...
*Built for NandaHack 2026 (MIT Media Lab × HCLTech, Project NANDA). The graveyard is public: ${base}/ in a browser.*
`;
}
